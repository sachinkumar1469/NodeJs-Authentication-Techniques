...
app.use("/",(req,res,next)=>{
    ...
    console.log("Custom property which is set on session",req.session.userId);
    ...
    if(userId){
        ...
        .then(result=>{
            res.render(path.join(require.main.filename,'..','views','home'),{user:result});
        })
        .catch(err=>{
            console.log("Unable to find user in home page");
        })
    } else {
        res.redirect('/signup');
    }
